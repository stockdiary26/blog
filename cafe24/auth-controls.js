(() => {
  ...
  authButton.onclick = async () => {
    ...
    try {
      ...
      let result;
      try { result = JSON.parse(text); } catch { throw new Error('로그아웃 응답 오류: HTTP ' + response.status); }
      if (!response.ok || result.authenticated !== false) throw new Error(result.error || '로그아웃하지 못했습니다.');
      auth = { ...auth, authenticated: false };
      $('#write').hidden = true;
      document.dispatchEvent(new Event('auth-state-changed'));
      for (const id of ['editor','taxonomyManager']) {
        const openDialog = document.getElementById(id);
        if (openDialog?.open) openDialog.close();
      }
      await refresh();
    } catch (error) { alert(error.message || '로그아웃 중 오류가 발생했습니다.'); }
