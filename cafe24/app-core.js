...
function renderCards(items, home = false) {
  return items.map(post => {
    ...
    const attachedImage = post.blocks?.find(block => block.type === 'image' && block.mediaUrl)?.mediaUrl;
    const videoText = [body, ...(post.blocks || []).filter(block => block.type === 'text').map(block => block.content)].join('\n');
    const video = window.youtubeVideos?.(videoText)[0];
    const thumbnail = attachedImage || video?.thumbnail || (post.mediaType?.startsWith('image') ? post.mediaUrl : null);
    const image = thumbnail ? `<img src="${esc(thumbnail)}" alt="${video && !attachedImage ? 'YouTube 동영상 썸네일' : ''}">` : '<div class="thumb-empty"></div>';
...
$('#adminButton').onclick = () => {
  ...
  if (!$('#setupKeyField')) {
    const field = document.createElement('div'); field.className = 'field'; field.id = 'setupKeyField';
    field.innerHTML = '<label for="setupKey">초기 설정 코드</label><input id="setupKey" autocomplete="off">';
    $('#authForm .actions').before(field);
  }
  $('#setupKeyField').hidden = auth.configured || !auth.setupKeyRequired;
  $('#setupKey').required = !auth.configured && !!auth.setupKeyRequired;
  $('#authForm').reset();
...
$('#authForm').onsubmit = async event => {
  ...
  const response = await fetch(auth.configured ? '/api/auth/login' : '/api/auth/setup', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({username:$('#authUser').value,password:$('#authPassword').value,setupKey:$('#setupKey')?.value || ''}) });
