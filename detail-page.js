(() => {
  ...
  function renderPostDetail(postId) {
    ...
    $('#app').innerHTML = `<div class="detail-page"><aside class="detail-sidebar"><div class="detail-sidebar-list">${categorySidebar()}</div></aside><article class="detail-article"><div class="detail-meta">${escapeHtml(name(post.category))} · ${post.date}</div><h1>${escapeHtml(post.title)}</h1>${body}<div class="detail-actions"><button type="button" class="btn detail-edit" data-detail-home>Home</button><button type="button" class="btn detail-edit" data-detail-list>List</button>${auth.authenticated ? '<button type="button" class="btn detail-edit" data-detail-edit>Edit</button>' : ''}</div></article></div>`;
    const edit = $('[data-detail-edit]');
    if (edit) edit.onclick = () => openPost(post);
    $('[data-detail-list]').onclick = () => openCategory(post.category);
    $('[data-detail-home]').onclick = () => navigate({});
  }
